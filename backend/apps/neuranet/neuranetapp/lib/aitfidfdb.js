...
exports.get_tfidf_db = async function(dbPath, lang="en", autosave=true, autosave_frequency=500) {
    const normPath = path.resolve(dbPath); if (!IN_MEM_DBS[normPath]) {    // load the DB from the disk only if needed
        try { fspromises.access(dbPath); } catch (err) {    // check the DB path exists or create it etc.
            if (err.code == "ENOENT") { 
                LOG.warn(`Unable to access the TF.IDF DB store at path ${path}. Creating a new one.`); fspromises.mkdir(path, {recursive: true});
            } else throw err;   // not an issue witht he DB folder, something else so throw it
        }
    ...
    return {    // TODO: add stream ingestion
        ...
        query: (query, topK, filter_function, cutoff_score, override_lang) => exports.query(query, topK, filter_function, override_lang||lang, cutoff_score, db),
...
exports.loadData = async function(path) {
    ...
    let tfidfDocStore = {}; try {tfidfDocStore = await fspromises.readFile(tfidfDocStorePath)} catch (err) {
        LOG.error(`TF.IDF search can't find or load ${TFIDF_FILE} from path ${path}. Using an empty document store.`);
    };
    let wordDocCounts = {}; try {await fspromises.readFile(wordDocCountsPath);} catch (err) {
        LOG.error(`TF.IDF search can't find or load ${WORDCOUNTS_FILE} from path ${path}. Using an empty vocabulary store.`);
    };
...
exports.ingest = exports.create = function(document, metadata, lang="en", db=EMPTY_DB) {
    const docIndex = _getHashIndex(metadata, lang), docWords = _getLangNormalizedWords(document, lang);
    db.tfidfDocStore[docIndex] = {metadata, scores: {}};
    const wordsCounted = {}; for (const word of docWords) {
        ...
        db.tfidfDocStore[docIndex].scores[word] = 0;   // dummy as we will rebuild the entire index in the next step, as IDF will change with every new doc as the word counts per doc changed (the denominator for IDF)
    }
...
exports.delete = function(metadata, lang="en", db=EMPTY_DB) {
    delete db.tfidfDocStore[_getHashIndex(metadata, lang)];
}

exports.update = (oldmetadata, newmetadata, lang="en", db=EMPTY_DB) => {
    const oldhash = _getHashIndex(oldmetadata, lang);
    db. tfidfDocStore[_getHashIndex(newmetadata)] = db.tfidfDocStore[oldhash];
    exports.delete(oldmetadata, lang, db);
}

exports.query = (query, topK, filter_function, lang="en", cutoff_score, db=EMPTY_DB) => {
    const queryWords = _getLangNormalizedWords(query, lang), scoredDocs = []; let totalScore = 0; 
    for (const document of Object.values(db.tfidfDocStore)) {
        ...
        const scoreThisDoc = 0; if (query) for (const queryWord of queryWords) if (document.scores[queryWord]) scoreThisDoc += document.scores[queryWord];
        scoredDocs.push({metadata: document.metadata, score: scoreThisDoc}); totalScore += scoreThisDoc;
    }
    if (query) scoredDocs = scoredDocs.sort((doc1, doc2) => doc1.score < doc2.score ? -1 : doc1.score > doc2.score ? 1 : 0);
    const cutoffDocs = []; if (cutoff_score) for (const scoredDocument of scoredDocs) {  // if cutoff_score is provided, then use it
        scoredDocument.score = scoredDocument.score/totalScore;
        if (scoredDocument.score >= cutoffDocs) cutoffDocs.push(scoredDocument);
...
function _recalculateTFIDF(db) {  // rebuilds the entire TF.IDF index for all documents, necessary as IDF changes with every new doc ingested
    for (const document of Object.values(db.tfidfDocStore)) for (const docWord of Object.keys(document.scores))
        document.scores[docWord] = _calculateTFIDF(docWord, Object.keys(document.scores), db);
}

function _calculateTFIDF(word, docWords, db) {
    let termcount = 0; for (const docWord of docWords) if (docWord == word) termcount++;
    const tf = termcount/docWords.length, idf = Math.log(db.tfidfDocStore.length/db.wordDocCounts[word]);
    return tf*idf;
}
...
const _getHashIndex = (metadata, lang="en") => {
    const lowerCaseObject = {}; for (const [index, value] of Object.entries(metadata))
        lowerCaseObject[index.toLocaleLowerCase(lang)] = value.toLocaleLowerCase(lang);
    crypto.createHash("md5").update(JSON.stringify(lowerCaseObject)).digest("hex");
}