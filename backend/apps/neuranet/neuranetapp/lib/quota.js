...
exports.checkQuota = async function(id, org) {
    ...
    LOG.info(`Found that ID ${id} from org ${org} is allowed a quota price equal to ${allowedQuota}.`);
    ...
    let totalUsedIn24Hours = 0; for (const model of models) {
        ...
        LOG.info(`ID ${id} from org ${org} used ${modelUsage} units of model ${model} in the last 24 hours, which equates to a price of ${priceOfUsageThisModel}.`);
    }
    ...
    if (totalUsedIn24Hours > allowedQuota) {
        ...
        LOG.info(`Quota use for ID ${id} from org ${org}, allowed = ${allowedQuota}, used = ${totalUsedIn24Hours}.`);
        return true;
    }
}